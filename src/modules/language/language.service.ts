...
export class LanguageService {
  ...
  async findPopularLanguages(limit: number = 3): Promise<Language[]> {
    // This query gets languages ordered by the number of movies that use them
    const query = `
      SELECT l.*, COUNT(m.id) as movie_count
      FROM "${modelNames.LANGUAGE_MODEL_NAME}" l
      JOIN "movie_spoken_languages" msl ON l.id = msl."language_id"
      JOIN "${modelNames.MOVIE_MODEL_NAME}" m ON m.id = msl."movie_id"
      GROUP BY l.id
