...
export class SemanticSearchStrategyOptimized extends BaseStrategy {
  ...
  async execute(input: StrategyInput): Promise<StrategyOutput> {
    ...
    try {
      ...
      if (!sanitized.isValid) {
        ...
        this.performanceMonitor.recordMetric('semantic_search_invalid_input', duration, {
      ...
      if (!normalized || normalized.trim().length === 0) {
        const duration = Date.now() - startTime;
        this.performanceMonitor.recordMetric('semantic_search_empty_input', duration, {
          sessionId: context.sessionId,
          language,
        });
      ...
      const cacheKey = this.generateSearchCacheKey(normalized, topK, similarityThreshold);
      ...
      if (results) {
        ...
        this.performanceMonitor.recordMetric('semantic_search_embedding', embeddingDuration, {
          sessionId: context.sessionId,
          language,
          cacheHit: false,
      ...
      if (!results || results.length === 0) {
        const duration = Date.now() - startTime;
        this.performanceMonitor.recordMetric('semantic_search_no_results', duration, {
          sessionId: context.sessionId,
          language,
          cacheHit,
        });
      ...
      if (filteredResults.length === 0) {
        const duration = Date.now() - startTime;
        this.performanceMonitor.recordMetric('semantic_search_all_suggested', duration, {
          sessionId: context.sessionId,
          language,
          cacheHit,
        });
      ...
      this.performanceMonitor.recordMetric('semantic_search_total', totalDuration, {
        sessionId: context.sessionId,
        language,
        cacheHit,
        embeddingDuration,
      ...
      const duration = Date.now() - startTime;
      this.performanceMonitor.recordError('semantic_search_error', error as Error, {
        sessionId: context.sessionId,
        language,
  ...
  private generateRecommendationText(
    ...
    try {
      if (language === 'vi') {
        if (movies.length === 1) {
          ...
          const year = movie.release_date
            ...
            : 'N/A';
          const genres = movie.genres?.map((g) => g.names[0]?.name).join(', ') || '';
          const director = movie.crew?.find((c: any) => c.job === 'Director')?.person?.name || '';
          ...
          const year1 = movie1.release_date ? new Date(movie1.release_date).getFullYear() : 'N/A';
          const year2 = movie2.release_date ? new Date(movie2.release_date).getFullYear() : 'N/A';
          const year3 = movie3?.release_date ? new Date(movie3.release_date).getFullYear() : 'N/A';
          
          const genres1 = movie1.genres?.map((g) => g.names[0]?.name).join(', ') || '';
          const genres2 = movie2.genres?.map((g) => g.names[0]?.name).join(', ') || '';
          const genres3 = movie3?.genres?.map((g) => g.names[0]?.name).join(', ') || '';
      ...
      const duration = Date.now() - startTime;
      this.performanceMonitor.recordMetric('recommendation_text_generation', duration, {
