...
export class MovieCrawlerService {
  ...
  async fetchAllMoviesToDatabase() {
    ...
    try {
      ...
      for (const language of topLanguages) {
        ...
        const firstPage = await api.get<{ total_pages: number }>('/discover/movie', { params: { ...params, page: 1 } });
        ...
        for (let page = 1; page <= totalPages; page++) {
          ...
          try {
            const { data } = await api.get<{ total_pages: number; results: Array<{ id: number; title: string; original_title: string; overview: string; release_date: string; poster_path?: string; backdrop_path?: string; genre_ids: number[]; }>; }>('/discover/movie', { params: { ...params, page } });
  ...
  async clearExistingData(queryRunner: QueryRunner) {
    await queryRunner.query(`TRUNCATE TABLE "${modelNames.ALTERNATIVE_TITLE_MODEL_NAME}" CASCADE`);
    await queryRunner.query(`TRUNCATE TABLE "${modelNames.IMAGE_MODEL_NAME}" CASCADE`);
    await queryRunner.query(`TRUNCATE TABLE "${modelNames.MOVIE_MODEL_NAME}" CASCADE`);
  }
  ...
  private async initializeLanguagesInBatches(): Promise<Language[]> {
    ...
    for (let i = 0; i < topLanguages.length; i += BATCH_SIZE) {
      ...
      const batchPromises = batch.map((lang) => this.languageService.findOrCreate({ iso_639_1: lang.code }).catch((error) => { console.error(`Failed to initialize language ${lang.code}:`, error); return null; }));
  ...
  private async initializeGenresForLanguage(language: Language): Promise<Map<number, Genre>> {
    ...
    try {
      const response = await api.get<{ genres: { id: number; name: string }[] }>('/genre/movie/list', { params: { language: language.iso_639_1 } });
      const genres = response.data.genres;
      for (let i = 0; i < genres.length; i++) {
        const genre = genres[i];
        if (!genre.name?.trim()) { console.warn(`Empty genre name for ID ${genre.id}, skipping...`); continue; }
        const genreName = genre.name.trim();
        let savedGenre = await this.genreRepository.findOne({ where: { original_id: genre.id } });
        if (!savedGenre) {
          const genreData = Genre.create(genreName, language.iso_639_1);
          savedGenre = this.genreRepository.create({ ...genreData, original_id: genre.id });
          ...
          if (existingNameIndex >= 0) { savedGenre.names[existingNameIndex].name = genreName; } else { savedGenre.names.push({ name: genreName, iso_639_1: language.iso_639_1 }); }
          savedGenre = await this.genreRepository.save(savedGenre);
        }
  ...
  private async processMovieImage(manager: EntityManager, url: string, alt: string): Promise<Image | null> {
    try {
      const result = await this.checkWithGlobe(url);
      if (!result) return null;
      const image = manager.create(Image, { url, alt, width: result.width, height: result.height, bytes: result.length });
  ...
  private async processMovieBatch(movies: TMDBDiscoverMovie[], language: Language, genreMap: Map<number, Genre>): Promise<Movie[]> {
    try {
      const processedMovies = await Promise.all(movies.map(async (movieData) => {
        try {
          const existingMovie = await this.movieRepository.findOne({ where: { original_id: movieData.id }, relations: ['genres', 'spoken_languages', 'original_language'] });
          if (existingMovie) {
            if (existingMovie.original_language.iso_639_1 !== language.iso_639_1) {
              await this.alternativeTitleService.importAlternativeTitles(existingMovie.id, [{ title: movieData.title, iso_639_1: language.iso_639_1, type: 'translation' }]);
              const hasLanguage = existingMovie.spoken_languages.some((lang) => lang.iso_639_1 === language.iso_639_1);
              if (!hasLanguage) { existingMovie.spoken_languages.push(language); await this.movieRepository.save(existingMovie); }
              return null;
            }
            return null;
          }
          const [poster, backdrop] = await Promise.all([
            movieData.poster_path ? this.processMovieImage(this.dataSource.manager, `https://image.tmdb.org/t/p/original${movieData.poster_path}`, movieData.title) : null,
            movieData.backdrop_path ? this.processMovieImage(this.dataSource.manager, `https://image.tmdb.org/t/p/original${movieData.backdrop_path}`, movieData.title) : null,
          ]);
          const movieGenres = movieData.genre_ids?.map((id: number) => genreMap.get(id)).filter((g): g is Genre => !!g) || [];
          return this.movieRepository.create({ title: movieData.title, original_title: movieData.original_title, overview: movieData.overview, release_date: movieData.release_date, spoken_languages: [language], original_language: language, poster: poster || undefined, backdrop: backdrop || undefined, genres: movieGenres, original_id: movieData.id });
        } catch (error) {
      ...
      if (validMovies.length > 0) { return this.movieRepository.save(validMovies); }
      return [];
    } catch (error) {
  ...
  private async fetchAlternativeTitlesAndOverviews(movieId: number): Promise<{ titles: { title: string; iso_639_1: string; type?: string }[]; overviews: { movieId: string; overview: string; iso_639_1: string }[]; }> {
    try {
      const results: { title: string; iso_639_1: string; type?: string }[] = [];
      const overviews: { movieId: string; overview: string; iso_639_1: string }[] = [];
      const altTitlesResponse = await api.get<{ titles?: Array<{ iso_3166_1?: string; title?: string; type?: string }>; }>(`/movie/${movieId}/alternative_titles`);
      if (altTitlesResponse.data && altTitlesResponse.data.titles && altTitlesResponse.data.titles.length) {
        ...
        for (const title of altTitlesResponse.data.titles) {
          if (title.iso_3166_1 && title.title) {
            ...
            if (existedLanguageCodes.includes(languageCode)) { continue; }
            existedLanguageCodes.push(languageCode);
            results.push({ title: title.title, iso_639_1: languageCode, type: title.type || 'alternative' });
            try {
              const movieDetails = await api.get<{ overview?: string; }>(`/movie/${movieId}`, { params: { language: languageCode } });
              if (movieDetails.data && movieDetails.data.overview) { overviews.push({ movieId: String(movieId), overview: movieDetails.data.overview, iso_639_1: languageCode }); }
            } catch (error) {
  ...
  async importAlternativeTitlesFromTMDB(movieId: string, tmdbId: number) {
    const movie = await this.movieRepository.findOne({ where: { id: movieId }, select: ['id', 'title', 'original_id'] });
    ...
    if (fetchedData.titles.length === 0) { return { message: 'No alternative titles or overviews found for this movie' }; }
    ...
    for (let i = 0; i < fetchedData.titles.length; i++) {
      ...
      if (titleItem.title) { alternativeTitles.push({ title: titleItem.title, iso_639_1: titleItem.iso_639_1, type: titleItem.type }); }
      if (overviewItem.overview) { alternativeOverviews.push({ overview: overviewItem.overview, iso_639_1: overviewItem.iso_639_1 }); }
    }
    const savedTitles = await this.alternativeTitleService.importAlternativeTitles(movieId, alternativeTitles);
    ...
    for (const overview of alternativeOverviews) {
      if (overview.overview && overview.iso_639_1) {
        const savedOverview = await this.alternativeOverviewService.saveAlternativeOverview(movieId, overview.overview, overview.iso_639_1);
        savedOverviews.push(savedOverview);
      }
    }
    return { message: `Successfully imported ${savedTitles.length} alternative titles and ${savedOverviews.length} alternative overviews`, titles: savedTitles, overviews: savedOverviews };
  }
  ...
  async updateMovieWithAlternativeTitles(movieId: string) {
    ...
    if (!tmdbId) { return { success: false, message: 'No TMDB ID found for this movie' }; }
    const fetchedData = await this.fetchAlternativeTitlesAndOverviews(tmdbId);
    if (fetchedData.titles.length === 0) { return { success: true, message: 'No alternative titles or overviews found for this movie', count: { titles: 0, overviews: 0 } }; }
    const alternativeTitles: { title: string; iso_639_1: string; type?: string; }[] = [];
    const alternativeOverviews: { overview: string; iso_639_1: string }[] = [];
    for (let i = 0; i < fetchedData.titles.length; i++) {
      const titleItem = fetchedData.titles[i];
    ...
    if (existingTitles.length > 0) { for (const title of existingTitles) { await this.alternativeTitleService.remove(title.id); } }
    const existingOverviews = await this.alternativeOverviewService.findAllByMovieId(movieId);
    if (existingOverviews.length > 0) { for (const overview of existingOverviews) { await this.alternativeOverviewService.remove(overview.id); } }
    const savedTitles = await this.alternativeTitleService.importAlternativeTitles(movieId, alternativeTitles);
    const savedOverviews: AlternativeOverview[] = [];
    for (const overview of alternativeOverviews) {
      if (overview.overview && overview.iso_639_1) {
    ...
    return { success: true, message: `Successfully updated movie with ${savedTitles.length} alternative titles and ${savedOverviews.length} alternative overviews`, count: { titles: savedTitles.length, overviews: savedOverviews.length }, titles: savedTitles, overviews: savedOverviews };
  }
  ...
  async importAlternativeTitlesAndOverviews(movieId: string, tmdbId: number) {
    ...
    if (titles.length === 0 && overviews.length === 0) { return { message: 'No alternative titles or overviews found for this movie' }; }
    const savedTitles = await this.alternativeTitleService.importAlternativeTitles(movieId, titles);
    const savedOverviews: AlternativeOverview[] = [];
