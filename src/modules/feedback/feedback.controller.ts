import { Body, Controller, Delete, Get, HttpCode, Param, ParseUUIDPipe, Patch, Post, Query, Req, UseGuards, UsePipes, ValidationPipe } from '@nestjs/common';
import { FeedbackService } from './feedback.service';
import { enums } from '@/common';
import { JwtAuthGuard } from '@/modules/auth/guards';
import { CreateFeedbackDto, GetCommentsQueryDto, UpdateFeedbackDto } from './feedback.dto';
...
export class FeedbackController {
  ...
  async create(
    ...
    const created = await this.feedbackService.create({ feedback: body.feedback, movie, user });
  ...
  async getByMovie(
    ...
    const [items, total] = await this.feedbackService.findByMovieIdPaginated(movieId, { skip, take });
  ...
  async update(
    ...
    if (existing.user.id !== req.user.sub) return ResponseUtil.error('Forbidden');
    const updated = await this.feedbackService.update(id, { feedback: body.feedback });
