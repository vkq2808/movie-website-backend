...
import { UploadMeta, UploadStatus } from '@/modules/video/video.service';
    ...
    try {
      ...
      for (const key of keys) {
        ...
        if (age > thresholdMs) {
          ...
          if (!sessionId) throw Error('sessionId not found');
          const dir = path.join(
            process.cwd(),
            'uploads',
            'tmp',
      ...
      this.logger.error('Error during upload cleanup', e);
    }
  }
}
